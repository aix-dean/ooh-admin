...
export default function ImmigrationProductsPage() {
  ...
  return (
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      ...
      <div className="flex items-center justify-between space-y-2">
        <div>
          <h2 className="text-3xl font-bold tracking-tight">Immigration Products</h2>
          <p className="text-muted-foreground">Manage immigration services, visa packages, and consultation products</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{products.totalCount}</div>
            <p className="text-xs text-muted-foreground">{products.totalPages} pages available</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{products.data.filter((p) => p.active).length}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{products.data.filter((p) => p.status === "PENDING").length}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div
              ...
              title={formatPrice(products.data.reduce((sum, p) => sum + p.price, 0))}
            >
              {products.data.reduce((sum, p) => sum + p.price, 0) > 999999
                ? `₱${(products.data.reduce((sum, p) => sum + p.price, 0) / 1000000).toFixed(1)}M`
                : products.data.reduce((sum, p) => sum + p.price, 0) > 999
                  ? `₱${(products.data.reduce((sum, p) => sum + p.price, 0) / 1000).toFixed(1)}K`
                  : formatPrice(products.data.reduce((sum, p) => sum + p.price, 0))}
            </div>
                ...
                <TableRow key={product.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <div className="flex items-center gap-3">
                      {product.media && product.media.length > 0 && (
                        <img
                          ...
                          onError={(e) => {
                            e.currentTarget.src = "/placeholder.svg?height=40&width=40&query=product"
                          }}
                        />
                      )}
                      <div>
                  ...
                  <TableCell>
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">{product.company_id}</div>
                    </div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    {product.specs_rental?.location && (
                      <div className="flex items-center text-sm">
                        ...
                        <span className="line-clamp-1 max-w-[150px]">{product.specs_rental.location}</span>
                      </div>
                    )}
                  </TableCell>
                  <TableCell>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <CardDescription className="text-sm">
                      <div>by {product.seller_name}</div>
                      <div className="text-xs text-muted-foreground mt-1">Company: {product.company_id}</div>
                    </CardDescription>
                  </div>
              ...
              <CardContent className="space-y-3">
                ...
                {product.media && product.media.length > 0 && (
                  <div className="aspect-video bg-gray-100 rounded-md overflow-hidden">
                    <img
                      ...
                      onError={(e) => {
                        e.currentTarget.src = "/placeholder.svg?height=120&width=200&query=product-placeholder"
                      }}
                    />
                  </div>
                )}
                ...
                {product.specs_rental?.location && (
                  <div className="flex items-center text-sm text-gray-500">
                    ...
                    <span className="line-clamp-1">{product.specs_rental.location}</span>
                  </div>
                )}
