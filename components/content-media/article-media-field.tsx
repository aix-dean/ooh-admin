...
interface ArticleMediaFieldProps {
  value: ArticleMediaItem[]
  onChange: (value: ArticleMediaItem[]) => void
  userId: string
  disabled?: boolean
}

export function ArticleMediaField({ value, onChange, userId, disabled = false }: ArticleMediaFieldProps) {
  const { toast } = useToast()
  const [isUploading, setIsUploading] = useState<Record<string, boolean>>({})
  const [uploadProgress, setUploadProgress] = useState<Record<string, number>>({})

  // Generate a unique ID for new items
  const generateId = () => `article-media-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
  ...
  const handleImageUpload = async (index: number, file: File) => {
    ...
    try {
      ...
      const storagePath = `article-media/${userId}/${fileName}`
      ...
      uploadTask.on(
        ...
        async () => {
          try {
            ...
            const newItems = [...value]
            newItems[index].imageUrl = downloadURL
        ...
        <Card key={item.id} className="overflow-hidden">
          <CardContent className="p-4 space-y-4">
            <div className="flex justify-between items-center">
              <h4 className="text-sm font-medium">Article Media #{index + 1}</h4>
              {value.length > 1 && (
                <Button
                  type="button"
                  variant="ghost"
              ...
              {item.imageUrl ? (
                ...
                <div>
                  <div
                    ...
                    onClick={() => !disabled && document.getElementById(`article-image-${index}`)?.click()}
                  >
                    <input
                      type="file"
