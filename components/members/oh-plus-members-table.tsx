...
import { type OHPlusMember, formatFullName } from "@/lib/members-service"
...
interface OHPlusMembersTableProps {
  members: OHPlusMember[]
  loading?: boolean
}

export function OHPlusMembersTable({ members, loading = false }: OHPlusMembersTableProps) {
  ...
  const getInitials = (member: OHPlusMember) => {
  ...
  return (
    <div className="space-y-6 overflow-visible">
      <div className="relative">
        <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
        <Input
          placeholder="Search OH! Plus members by name, email, company name, or company ID..."
          value={searchQuery}
      ...
      {filteredMembers.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-lg shadow-sm border border-gray-100">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-1">No OH! Plus members found</h3>
                ...
                <CardContent className="p-0">
                    ...
                    <div className="flex items-center gap-3 w-full min-w-0">
                      ...
                      <div className="flex items-center gap-2 min-w-0 flex-1">
                        <h3
                          ...
                          onClick={() => {
                            window.location.href = `/dashboard/immigration/members/${member.id}?platform=oh-plus`
                          }}
                        >
                          {member.displayName ||
                            formatFullName(member.firstName, member.middleName, member.lastName) ||
                  ...
                  <div className="p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      {member.productsCount && (
                        <div className="text-xs text-muted-foreground flex items-center">
                          <span className="ml-6">
                            M: {member.productsCount.merchandise || 0}, R: {member.productsCount.rental || 0}
                          </span>
                        </div>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm truncate">{formatLocation(member.location)}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2 pt-1 border-t border-gray-100">
                      <div className="flex items-center gap-2">
                        <Calendar className="h-4 w-4 text-muted-foreground shrink-0" />
                        <span className="text-xs text-muted-foreground">
                          Created: {formatDate(member.created || member.createdTime)}
                        </span>
                      </div>
                      {member.activeDate && (
                        <div className="flex items-center gap-2">
